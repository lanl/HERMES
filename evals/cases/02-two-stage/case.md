...
which cluster into 92,703 photons at the settings in `input/config.yaml`.
...
- `save_photon_pixels` is `true`, so the run also writes a pixel-to-cluster
  ...
  belongs to for reconstruction diagnostics. Here that table holds 482,699
...
- The counts in both summaries are real, taken from an actual two-stage run of
  this file: 505,721 pixels form 100,969 clusters, 8,266 are rejected, leaving
  92,703 photons.
