...
measurements happen yet — that is the later examples.
...
This example does not need the camera, so `tcp_ip` is left unset (SERVAL will try
to autodiscover a camera, but nothing here depends on it connecting).

## Run it

```bash
pixi run python examples/acquisition/serval/server_control/run_serval_control.py
```

Pass another HERMES YAML file as the optional argument to use your own config:

```bash
pixi run python examples/acquisition/serval/server_control/run_serval_control.py /path/to/config.yaml
```
