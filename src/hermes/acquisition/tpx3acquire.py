...
from hermes.acquisition.tpx3zaber import set_zaber_ao
from hermes.acquisition.tpx3serval import start_serval_server, stop_serval_server
from hermes.acquisition.tpx3logging import log_info
...
def run_tpx3_operations(config: Dict[str, Any], verbose: int = 1):
    ...
    for i in range(num_runs):
        ...
        tpx3serval.set_and_load_server_destination(config, verbose_level=verbose)
        tpx3serval.log_info(config, http_string=f'/dashboard?run={run_number}', verbose_level=verbose)
        tpx3serval.take_exposure(config, verbose_level=verbose)
