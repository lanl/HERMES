...
// that can no longer link to any future photon. cell_width is a lookup
// accelerator only: any cell_width at least the linking radius produces the
// same photon clusters. on_photon_cluster receives the member photon indices
// (into the photons vector, ascending) of each cluster as it closes, in the
// order clusters close; any still-open clusters are flushed at end of input in
// earliest-photon-time order.
void clusterPhotons(
